...
export default function CurriculumManager() {
  ...
  const fetchCourseAndLessons = async () => {
    try {
      ...
      setError(err.response?.status === 404 ? 'Course not found. It may have been deleted or the slug changed.' : 'Failed to load course details.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (error || !course) return (
    <div className="max-w-xl mx-auto py-24 px-6 text-center">
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-2">{error || 'Course not found'}</h2>
      <p className="text-gray-500 mb-8">We couldn't retrieve the curriculum for this course. Please verify the URL or try again from the dashboard.</p>
      <Link to="/office/portal" className="px-6 py-3 bg-mylms-purple text-white font-bold rounded-lg hover:bg-mylms-purple/90 transition-all shadow-md inline-block">
